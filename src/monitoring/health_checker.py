...
class HealthChecker:
        ...
        try:
            ...
            if cache_test['success']:
                ...
                if cache_test['response_time_ms'] > self.response_time_threshold:
                    status = HealthStatus.DEGRADED
                    message = f"Cache slow response: {cache_test['response_time_ms']:.1f}ms"
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=self.timeout)) as session:
                    async with session.get(endpoint['url']) as response:
                        ...
                        checks.append(HealthCheck(
                            ...
                            timestamp=datetime.utcnow(),
                            details={'http_status': response.status, 'url': endpoint['url']}
                        ))
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_overall_status': recent_checks[-1]['overall_status'] if recent_checks else 'unknown',
            'average_response_time_ms': sum(c['total_time_ms'] for c in recent_checks) / len(recent_checks) if recent_checks else 0,
            'uptime_percentage': len([c for c in recent_checks if c['overall_status'] == 'healthy']) / len(recent_checks) * 100 if recent_checks else 0
        }