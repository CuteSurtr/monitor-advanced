...
class AIMLEngine:
    ...
    async def engineer_features(self, symbol: str, lookback_days: int = 252) -> pd.DataFrame:
        ...
        try:
            ...
            stock_data = await self.db_manager.get_stock_data(symbol, start_time, end_time)
    ...
    def _add_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Volatility rank
        df['vol_rank_60'] = returns.rolling(window=60).std().rolling(window=252).rank(pct=True)
        
        return df
    
    async def _add_market_regime_features(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        """Add market regime and macro features."""
        try:
            # VIX data (if available)
            vix_data = await self.db_manager.get_market_indicator_data("VIX", df.index[0], df.index[-1])
            if vix_data:
                vix_df = pd.DataFrame(vix_data)
                vix_df['timestamp'] = pd.to_datetime(vix_df['timestamp'])
                vix_df.set_index('timestamp', inplace=True)
                df = df.join(vix_df[['value']].rename(columns={'value': 'vix'}), how='left')
            ...
            if symbol != 'SPY':
                spy_data = await self.db_manager.get_stock_data("SPY", df.index[0], df.index[-1])
                if spy_data:
                    ...
                    df['beta'] = df['close'].pct_change().rolling(window=60).cov(spy_returns) / spy_returns.rolling(window=60).var()
    ...
    async def _add_sentiment_features(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            sentiment_data = await self.db_manager.get_sentiment_data(symbol, df.index[0], df.index[-1])
            if sentiment_data:
    ...
    async def _add_cross_asset_features(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            for related_symbol in related_symbols[:2]:  # Limit to avoid too many features
                related_data = await self.db_manager.get_stock_data(related_symbol, df.index[0], df.index[-1])
                if related_data:
                    ...
                    # Rolling correlation
                    df[f'corr_{related_symbol}_30'] = symbol_returns.rolling(window=30).corr(related_returns)
    ...
    async def train_ensemble_models(self, symbol: str, prediction_horizon: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            df['target'] = df['close'].shift(-prediction_horizon).pct_change(prediction_horizon)
            df = df.dropna()
            ...
            if TENSORFLOW_AVAILABLE:
                lstm_performance = await self._train_lstm_model(X_train_scaled, y_train, X_test_scaled, y_test, symbol)
                if lstm_performance:
            ...
            rf_importance = dict(zip(self.feature_columns, rf_model.feature_importances_))
            xgb_importance = dict(zip(self.feature_columns, xgb_model.feature_importances_))
            
            # Average feature importance
            self.feature_importance[symbol] = {}
            for feature in self.feature_columns:
                avg_importance = (rf_importance.get(feature, 0) + xgb_importance.get(feature, 0)) / 2
    ...
    async def _train_lstm_model(self, X_train: np.ndarray, y_train: np.ndarray, 
                              X_test: np.ndarray, y_test: np.ndarray, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Build LSTM model
            model = Sequential([
                LSTM(config['lstm_units'], return_sequences=True, input_shape=(seq_length, X_train.shape[1])),
    ...
    async def generate_predictions(self, symbol: str, prediction_horizon: int = 24) -> List[MLPrediction]:
                ...
                try:
                    ...
                    if model_name == 'lstm' and TENSORFLOW_AVAILABLE:
                        ...
                        if len(df) >= seq_length:
                            sequence_data = df[self.feature_columns].iloc[-seq_length:].values
                            sequence_scaled = self.scalers[symbol].transform(sequence_data)
    ...
    async def generate_trading_signals(self, symbol: str) -> Optional[TradingSignal]:
        ...
        try:
            ...
            weighted_prediction = sum(pred.predicted_value * pred.confidence for pred in predictions) / total_weight
