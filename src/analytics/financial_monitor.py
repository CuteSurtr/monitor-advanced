...
class GlobalFinancialMonitor:
    ...
    async def collect_realtime_market_data(self):
        ...
        try:
            ...
            self.logger.info(f"Collected market data for {len(market_data_batch)} symbols")
                    ...
                    for i, symbol in enumerate(futures_symbols):
                        try:
                            ...
                            if not future_data.empty:
                                vix_futures[f'M{i+1}'] = float(future_data['Close'].iloc[-1])
                        except:
                            continue
                except:
                    pass
    ...
    async def calculate_portfolio_analytics(self, portfolio_id: str = None):
        ...
        try:
            ...
            # Calculate VaR/CVaR
            var_cvar_metrics = await self.risk_engine.calculate_portfolio_var_cvar(portfolio_id)
            
            # Calculate max drawdown
            drawdown_metrics = await self.risk_engine.calculate_max_drawdown(portfolio_id)
