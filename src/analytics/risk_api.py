...
async def get_portfolio_volatility(
    ...
    """
    try:
        risk_metrics = await risk_analytics.calculate_portfolio_risk(portfolio_id, lookback_days)
...
@router.get("/portfolio/{portfolio_id}/comprehensive", response_model=RiskMetricsResponse)
...
async def get_asset_risk_metrics(
    ...
    try:
        ...
        if not risk_data:
            raise HTTPException(status_code=404, detail=f"No data found for asset: {symbol}")
        ...
        return AssetRiskResponse(
            ...
            timestamp=datetime.fromisoformat(risk_data.get('timestamp', datetime.now().isoformat()))
        )
        
    except Exception as e:
        logger.error(f"Error calculating asset risk for {symbol}: {e}")
        raise HTTPException(status_code=500, detail=f"Error calculating asset risk: {str(e)}")
...
async def run_stress_test(
    ...
    """
    try:
        stress_results = await risk_analytics.calculate_stress_test(portfolio_id, shock_scenarios)
        
        if not stress_results:
            raise HTTPException(status_code=404, detail=f"Portfolio {portfolio_id} not found or no positions")
...
async def get_correlation_matrix(
    ...
    """
    try:
        correlation_matrix = await risk_analytics.calculate_portfolio_correlation_matrix(
