...
class RiskAnalytics:
    ...
    async def calculate_portfolio_risk(self, portfolio_id: int, 
        ...
        try:
            ...
            asset_returns = await self._get_portfolio_historical_data(positions, lookback_days)
            ...
            self.logger.error(f"Error calculating portfolio risk for {portfolio_id}: {e}")
            return RiskMetrics()
    
    async def calculate_asset_risk(self, symbol: str, 
                                 lookback_days: int = 252,
        ...
        try:
            ...
            if symbol.endswith('=X'):  # Forex
                ...
                data = await self.db_manager.get_commodity_data(symbol, start_time, end_time)
            else:  # Stocks
            ...
            for conf_level in confidence_levels:
                for method in ['historical', 'parametric', 'monte_carlo']:
                    ...
                    risk_metrics[f'var_{int(conf_level*100)}_{method}'] = var_cvar['var']
                    risk_metrics[f'cvar_{int(conf_level*100)}_{method}'] = var_cvar['cvar']
            
            # Volatility metrics
            for method in ['historical', 'ewma', 'garch']:
                vol_metrics = await self.analytics_engine.calculate_portfolio_volatility(
                    returns.to_frame(), method
                )
                risk_metrics[f'volatility_{method}'] = vol_metrics['volatility']
            
            # Sharpe ratio
            risk_metrics['sharpe_ratio'] = await self.analytics_engine.calculate_sharpe_ratio(
                returns.to_frame()
            )
            
            # Maximum drawdown
            values = (1 + returns).cumprod()
            drawdown_metrics = await self.analytics_engine.calculate_max_drawdown(values)
    ...
    async def calculate_portfolio_correlation_matrix(self, portfolio_id: int,
        ...
        try:
            ...
            return await self.analytics_engine.calculate_correlation_matrix(asset_returns)
            
        except Exception as e:
            self.logger.error(f"Error calculating correlation matrix: {e}")
            return pd.DataFrame()
    
    async def calculate_stress_test(self, portfolio_id: int,
        ...
        try:
            ...
            impact_percentage = (total_impact / current_value) if current_value > 0 else 0
    ...
    async def get_risk_dashboard_data(self, portfolio_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            correlation_matrix = await self.calculate_portfolio_correlation_matrix(portfolio_id)
    ...
    async def _calculate_all_risk_metrics(self, portfolio_returns: pd.Series, 
        ...
        try:
            ...
            drawdown_metrics = await self.analytics_engine.calculate_max_drawdown(portfolio_values)
            ...
            # Correlation matrix
            correlation_matrix = await self.analytics_engine.calculate_correlation_matrix(asset_returns)
    ...
    async def _generate_risk_alerts(self, risk_metrics: RiskMetrics) -> List[Dict[str, Any]]:
    ...
    async def _get_portfolio_historical_data(self, positions: List[Dict[str, Any]], 
    ...
    async def _calculate_portfolio_returns(self, asset_returns: Dict[str, pd.Series], 
        ...
        return await self.analytics_engine._calculate_portfolio_returns(asset_returns, weights)
