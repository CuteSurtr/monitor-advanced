...
class OptionsAnalyzer:
    ...
    def __init__(self):
        ...
        self.monitored_symbols = [
            'SPY', 'QQQ', 'IWM',  # ETFs
            'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA',  # Large caps
            'NVDA', 'META', 'NFLX'  # Tech
        ]
                    ...
                    if not option_chain.calls.empty:
                        for _, call in option_chain.calls.iterrows():
                            greeks = self._calculate_greeks(
                                current_price, call['strike'], expiry_date, 
    ...
    def _calculate_greeks(self, spot: float, strike: float, expiry: datetime, 
        ...
        try:
            ...
            theta = ((-spot * norm.pdf(d1) * iv / (2 * np.sqrt(t)) 
                     - self.risk_free_rate * strike * np.exp(-self.risk_free_rate * t) * 
    ...
    async def analyze_options_chain(self, symbol: str) -> Optional[OptionsChainAnalysis]:
        ...
        try:
            ...
            ticker = yf.Ticker(symbol)
            stock_info = ticker.history(period="1d")
            underlying_price = stock_info['Close'].iloc[-1] if not stock_info.empty else 0
    ...
    def _calculate_max_pain(self, options_data: List[OptionData], underlying_price: float) -> float:
