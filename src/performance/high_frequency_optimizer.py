...
class HighFrequencyOptimizer:
    ...
    async def register_stream(self, symbol: str, callback: Callable[[Dict], Coroutine]) -> bool:
        """
        ...
        try:
            if len(self.active_streams) >= self.max_concurrent_streams:
                self.logger.warning(f"Maximum concurrent streams ({self.max_concurrent_streams}) reached")
                return False
    ...
    async def process_message(self, symbol: str, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            metrics.avg_latency_ms = (metrics.avg_latency_ms * 0.9 + processing_time * 0.1)
    ...
    async def _update_system_metrics(self):
        ...
        try:
            ...
            if len(self.performance_history) > 0:
                prev_total = sum(m['metrics'].get('total_messages', 0) for m in self.performance_history[-60:])
                self.system_metrics.messages_per_second = (total_messages - prev_total) / 60
    ...
    async def _optimize_performance(self):
        ...
        try:
            ...
            avg_latency = np.mean([m.avg_latency_ms for m in self.stream_metrics.values()])
    ...
    async def _calculate_cache_metrics(self):
        ...
        try:
            if self.redis_pool:
                ...
                self.system_metrics.cache_hit_rate = (hits / total * 100) if total > 0 else 0
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            ...
            avg_latency = np.mean([m.avg_latency_ms for m in self.stream_metrics.values()])
            error_rate = sum(m.error_count for m in self.stream_metrics.values()) / max(sum(m.message_count for m in self.stream_metrics.values()), 1)
            
            return (
                avg_latency < self.target_latency_ms * 2 and
                error_rate < 0.01 and  # Less than 1% error rate
                self.system_metrics.cpu_usage < 90 and
                self.system_metrics.memory_usage_mb < 2048  # Less than 2GB
            )
