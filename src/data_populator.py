...
class MarketDataPopulator:
    ...
    def __init__(self):
        ...
        self.base_prices = {
            'AAPL': 175.0, 'MSFT': 335.0, 'GOOGL': 2850.0, 'AMZN': 3250.0, 'TSLA': 240.0,
            'META': 485.0, 'NVDA': 875.0, 'JPM': 148.0, 'JNJ': 162.0, 'SPY': 445.0,
            'BTC': 67000.0, 'ETH': 3800.0, 'USDT': 1.0, 'BNB': 610.0, 'USDC': 1.0,
            'EUR/USD': 1.0850, 'GBP/USD': 1.2750, 'USD/JPY': 150.25, 'USD/CHF': 0.9180,
            'XAU/USD': 2650.0, 'XAG/USD': 31.50, 'WTI': 82.50, 'BRENT': 85.75
        }
                ...
                with conn.cursor() as cursor:
                    ...
                    for stock_id, symbol in stocks:
                        if symbol in self.current_prices:
                            ...
                            price_change = random.gauss(0, current_price * 0.01)  # 1% volatility
                            new_price = max(current_price + price_change, current_price * 0.95)  # Don't drop below 95%
                            ...
                            cursor.execute(insert_query, (
                                stock_id, datetime.now().replace(microsecond=0),
                ...
                for stock_id, symbol in stocks:
                    if symbol in self.current_prices:
                        ...
                        for _ in range(random.randint(5, 20)):  # 5-20 trades per cycle
                            price = base_price * (1 + random.gauss(0, 0.001))  # Small price variation
                            ...
                            cursor.execute(tick_query, (
                                stock_id, 'stock', datetime.now(),
                                price, volume, aggressor_side
                            ))
                        ...
                        for side in ['BID', 'ASK']:
                            for level in range(1, 6):  # 5 levels deep
                                if side == 'BID':
                                    price_level = base_price * (1 - level * 0.0001)  # Slightly below
                                else:
                                    price_level = base_price * (1 + level * 0.0001)  # Slightly above
                                ...
                                cursor.execute(ob_query, (
                                    stock_id, 'stock', datetime.now(),
                                    side, price_level, size
                                ))
                        
                        # Generate microstructure features
                        bid_ask_spread = base_price * random.uniform(0.0001, 0.001)  # 1-10 bps
                        ...
                        cursor.execute(micro_query, (
                            stock_id, 'stock', datetime.now(),
                            bid_ask_spread, order_flow_imbalance, trade_intensity
                        ))
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if random.random() < 0.3:  # 30% chance of trade
                    ...
                    # Get current price
                    cursor.execute("SELECT symbol FROM market_data.stocks WHERE id = %s", (stock_id,))
                    ...
                    cursor.execute(trans_query, (
                        ...
                        stock_id, transaction_type, quantity, price,
                    ...
                    """, (price, price, '00000000-0000-0000-0000-000000000001', stock_id))
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if random.random() < 0.1:  # 10% chance
                    alert_types = [
                        ('Portfolio Loss Alert', 'HIGH', 'Daily P&L exceeded -5% threshold'),
                        ('Volatility Spike', 'MEDIUM', 'Market volatility increased significantly'),
                        ('Position Concentration', 'LOW', 'Single position exceeds 30% of portfolio'),
                    ...
                    cursor.execute(alert_query, (title, severity, message, datetime.now()))
                ...
                for stock_id, symbol in stocks:
                    if symbol in self.current_prices:
                        ...
                        for indicator_name, value in indicators.items():
                            ...
                            """, (stock_id, indicator_name, value, datetime.now().replace(microsecond=0)))
...
async def main():
    ...
    try:
        ...
        self.logger.info("🔄 Running initial data population...")
        for i in range(3):
        ...
        self.logger.info("📊 Data population stopped by user")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print("This will generate real-time market data for your Grafana dashboards")
