        ...
        for rule in alert_rules:
            try:
                ...
                if alert_triggered:
                    ...
                    alerts_triggered += 1
                    logger.info(f"Alert triggered for {rule['symbol']}: {rule['rule_type']}")
                
            except Exception as e:
                logger.error(f"Error processing alert rule {rule.get('id')}: {e}")
                continue
        
        logger.info(f"Processed {rules_processed} rules, triggered {alerts_triggered} alerts")
        return {
            'success': True,
...
def send_alert_notification(self, alert_data: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"Sending notification for alert: {alert_data.get('symbol')} - {alert_data.get('alert_type')}")
...
def check_system_alerts(self):
    ...
    try:
        ...
        logger.info(f"System check complete, {system_alerts_triggered} alerts triggered")
        return {
            'success': True,
