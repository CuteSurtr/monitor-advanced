    ...
    try:
                ...
                for worker, stat_data in stats.items():
                    if worker in worker_stats['workers']:
                        worker_stats['workers'][worker].update({
                            ...
                            'pool_processes': stat_data.get('pool', {}).get('processes', 0),
        ...
        for worker, data in worker_stats['workers'].items():
            ...
            if data.get('active_tasks', 0) > 50:  # Too many active tasks
                unhealthy_workers.append(f"{worker}: too many active tasks ({data['active_tasks']})")
        ...
        logger.info(f"Celery monitoring complete: {worker_stats['active_workers']} active workers")
        return worker_stats
