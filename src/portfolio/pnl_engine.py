...
class PnLEngine:
    ...
    async def calculate_portfolio_pnl(self, portfolio_id: int, 
                                    calculate_historical: bool = False) -> PnLSnapshot:
        """
        ...
        try:
            ...
            self.logger.error(f"Error calculating P&L for portfolio {portfolio_id}: {e}")
            return self._create_empty_snapshot(portfolio_id)
    ...
    async def calculate_position_pnl_detailed(self, portfolio_id: int, 
        ...
        try:
            ...
            if not position_data:
                return {'error': f'Position {symbol} not found in portfolio {portfolio_id}'}
            ...
            historical_pnl = await self._get_position_historical_pnl(portfolio_id, symbol)
            ...
            return {
                ...
                'unrealized_pnl_pct': float((position.unrealized_pnl / position.cost_basis * 100) if position.cost_basis != 0 else 0),
    ...
    async def calculate_sector_pnl(self, portfolio_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for sector, data in sector_pnl.items():
                allocation_pct = (data['market_value'] / total_market_value * 100) if total_market_value > 0 else 0
    ...
    async def calculate_performance_attribution(self, portfolio_id: int, 
        ...
        try:
            ...
            portfolio_return = (snapshot.total_pnl / snapshot.total_cost_basis) if snapshot.total_cost_basis > 0 else Decimal('0')
            ...
            for position in snapshot.positions:
                position_return = (position.total_pnl / position.cost_basis) if position.cost_basis > 0 else Decimal('0')
                weight = (position.market_value / snapshot.total_market_value) if snapshot.total_market_value > 0 else Decimal('0')
            ...
            return {
                ...
                'position_attribution': sorted(position_attribution, key=lambda x: x['contribution'], reverse=True),
                'timestamp': snapshot.timestamp.isoformat()
            }
    ...
    async def _calculate_position_pnl(self, position_data: Dict[str, Any]) -> Position:
        ...
        try:
            ...
            day_pnl = quantity * (current_price - Decimal(str(previous_close))) if previous_close else Decimal('0')
    ...
    def _aggregate_portfolio_pnl(self, portfolio_id: int, positions: List[Position]) -> PnLSnapshot:
    ...
    async def _get_latest_commodity_price(self, symbol: str) -> Optional[Dict[str, Any]]:
    ...
    def _get_top_performers(self, positions: List[Position], 
                          performance_type: str = 'gainers', limit: int = 5) -> List[Dict[str, Any]]:
        """Get top performing positions."""
        try:
            # Sort by unrealized P&L percentage
            sorted_positions = sorted(
                positions, 
                key=lambda p: (p.unrealized_pnl / p.cost_basis) if p.cost_basis > 0 else 0,
    ...
    async def _get_position_historical_pnl(self, portfolio_id: int, symbol: str) -> List[Dict[str, Any]]:
    ...
    async def _get_benchmark_return(self, benchmark_symbol: str) -> float:
        ...
        try:
            ...
            data = await self.db_manager.get_stock_data(benchmark_symbol, start_time, end_time)
            if len(data) >= 2:
