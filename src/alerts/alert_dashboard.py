...
class AlertDashboard:
        ...
        return html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    html.P("Monitor and manage stock market alerts", className="text-muted")
                ])
            ]),
            
            # Statistics Cards
            dbc.Row([
                dbc.Col(self._create_stat_card("Total Conditions", "total_conditions", "primary"), width=3),
                dbc.Col(self._create_stat_card("Active Conditions", "enabled_conditions", "success"), width=3),
                dbc.Col(self._create_stat_card("Total Events", "total_events", "warning"), width=3),
                dbc.Col(self._create_stat_card("Pending Events", "pending_events", "danger"), width=3),
            ], className="mb-4"),
            ...
            dbc.Tabs([
                ...
                ], label="Alert Conditions", tab_id="conditions"),
                
                # Alert Events Tab
                dbc.Tab([
                    self._create_events_tab()
                ], label="Alert Events", tab_id="events"),
                
                # Statistics Tab
                dbc.Tab([
                    self._create_statistics_tab()
                ], label="Statistics", tab_id="statistics"),
                
                # Settings Tab
                dbc.Tab([
                    self._create_settings_tab()
                ], label="Settings", tab_id="settings"),
            ], id="alert-tabs", active_tab="conditions"),
    ...
    def _create_stat_card(self, title: str, data_key: str, color: str) -> dbc.Card:
        ...
        return dbc.Card([
            dbc.CardBody([
                html.H4(id=f"stat-{data_key}", children="0", className=f"text-{color}"),
    ...
    def _create_conditions_tab(self) -> html.Div:
        ...
        return html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Button("Add New Alert", id="add-alert-btn", color="primary", className="me-2"),
                    dbc.Button("Refresh", id="refresh-conditions-btn", color="secondary", className="me-2"),
                    dbc.Switch(id="monitoring-switch", label="Enable Monitoring", className="me-2"),
                ], width=6),
                dbc.Col([
                    dcc.Dropdown(
                        id="symbol-filter",
            ...
            ], id="conditions-table", striped=True, bordered=True, hover=True)
        ])
    ...
    def _create_events_tab(self) -> html.Div:
        ...
        return html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Button("Refresh", id="refresh-events-btn", color="secondary", className="me-2"),
                    dcc.DatePickerRange(
            ...
            ], id="events-table", striped=True, bordered=True, hover=True)
        ])
    ...
    def _create_statistics_tab(self) -> html.Div:
        ...
        return html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    dcc.Graph(id="events-timeline-chart")
                ], width=12)
            ])
        ])
                            ...
                            dbc.Form([
                                dbc.Row([
                                    dbc.Col([
                                        ...
                                        dbc.Switch(id="email-notifications", label="Enable", value=True)
                                    ], width=6),
                                    dbc.Col([
                                        dbc.Label("Telegram Notifications"),
                                        dbc.Switch(id="telegram-notifications", label="Enable", value=False)
                                    ], width=6)
                                ], className="mb-3"),
                                ...
                                dbc.Row([
                                    dbc.Col([
                                        ...
                                        dbc.Switch(id="slack-notifications", label="Enable", value=False)
                                    ], width=6),
                                    dbc.Col([
                                        dbc.Label("Webhook Notifications"),
                                        dbc.Switch(id="webhook-notifications", label="Enable", value=False)
                                    ], width=6)
                                ], className="mb-3"),
                                
                                dbc.Row([
                                    dbc.Col([
                                        dbc.Label("Default Cooldown (minutes)"),
                                        dbc.Input(id="default-cooldown", type="number", value=30, min=1)
                                    ], width=6),
                                    dbc.Col([
                                        dbc.Label("Auto-refresh Interval (seconds)"),
                                        dbc.Input(id="refresh-interval", type="number", value=30, min=5)
                                    ], width=6)
                                ])
                            ])
                        ])
                        ...
                        dbc.CardBody([
                            dbc.Form([
                                dbc.Row([
                                    dbc.Col([
                                        dbc.Label("Symbol"),
                                        dbc.Input(id="test-symbol", placeholder="AAPL", value="AAPL")
                                    ], width=4),
                                    dbc.Col([
                                ...
                                dbc.Button("Send Test Notification", id="test-notification-btn", color="warning")
                            ])
                        ])
                    ])
                ], width=6)
    ...
    def _create_alert_modal(self) -> dbc.Modal:
            ...
            dbc.ModalBody([
                dbc.Form([
                    dbc.Row([
                        dbc.Col([
                            dbc.Label("Symbol"),
                            dbc.Input(id="modal-symbol", type="text", placeholder="AAPL")
                        ], width=6),
                        dbc.Col([
                            dbc.Label("Alert Type"),
                            dcc.Dropdown(
                                ...
                                options=[
                                    {"label": "Price Threshold", "value": "price_threshold"},
                                    {"label": "Price Change", "value": "price_change"},
                                    {"label": "Technical Indicator", "value": "technical_indicator"},
                                    {"label": "Volume Spike", "value": "volume_spike"},
                                    {"label": "Anomaly Detected", "value": "anomaly_detected"}
                                ]
                            )
                    ...
                    dbc.Row([
                        dbc.Col([
                            dbc.Label("Condition"),
                            dbc.Input(id="modal-condition", type="text", placeholder="price > 100")
                        ], width=6),
                        dbc.Col([
                            dbc.Label("Threshold"),
                            dbc.Input(id="modal-threshold", type="number", step="0.01")
                        ], width=6)
                    ], className="mb-3"),
                    ...
                    dbc.Row([
                        ...
                        dbc.Col([
                            ...
                            dbc.Input(id="modal-cooldown", type="number", value=30, min=1)
                        ], width=6)
                    ], className="mb-3"),
                    ...
                    dbc.Row([
                        dbc.Col([
                            ...
                            dbc.Textarea(id="modal-custom-message", placeholder="Custom alert message...")
                        ], width=12)
                    ])
                ])
            ]),
        ...
        ], id="alert-modal", size="lg", is_open=False)
    ...
    def register_callbacks(self, app: dash.Dash):
        ...
        def refresh_data(n_intervals, refresh_conditions, refresh_events):
            ...
            try:
                # Get conditions
                conditions_response = requests.get(f"{self.api_base_url}/alerts/conditions")
        ...
        @app.callback(
            ...
            [Input("alert-conditions-store", "data"),
        ...
        def update_conditions_table(conditions_data, symbol_filter):
            ...
            if symbol_filter:
                conditions_data = [c for c in conditions_data if c["symbol"] in symbol_filter]
        ...
        def update_events_table(events_data, symbol_filter, start_date, end_date):
            ...
            if start_date and end_date:
                ...
                events_data = [
                    ...
                    if start_dt <= datetime.fromisoformat(e["triggered_at"].replace("Z", "+00:00")) <= end_dt
                ]
            ...
            for event in events_data:
                ...
                actions = html.Div([
                    dbc.Button("Acknowledge", size="sm", color="success", className="me-1") 
                    if not event["acknowledged"] else html.Span(""),
                ...
                rows.append(html.Tr([
                    ...
                    html.Td(severity_badge),
                    html.Td(event["message"][:50] + "..." if len(event["message"]) > 50 else event["message"]),
                    html.Td(status_badge),
                    html.Td(actions)
                ]))
        ...
        def save_alert(save_clicks, symbol, alert_type, condition, threshold, 
                      severity, cooldown, notification_methods, custom_message):
        ...
        @app.callback(
            ...
            [Input("monitoring-switch", "value")]
        )
        def toggle_monitoring(value):
            """Toggle alert monitoring"""
            try:
                if value:
                    response = requests.post(f"{self.api_base_url}/alerts/monitoring/start")
                else:
                    response = requests.post(f"{self.api_base_url}/alerts/monitoring/stop")
...
def create_alert_dashboard(api_base_url: str = "http://localhost:8080") -> AlertDashboard:
    """
