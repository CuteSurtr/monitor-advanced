...
class CryptoDataCollector:
    ...
    def __init__(self, config, db_manager, cache_manager: CacheManager):
        ...
        self.crypto_symbols = [
            ...
            'DOT-USD', 'AVAX-USD', 'SHIB-USD', 'LTC-USD', 'TRX-USD', 'UNI-USD', 'LINK-USD',
            'XLM-USD', 'BCH-USD', 'NEAR-USD', 'ALGO-USD', 'XMR-USD', 'VET-USD',
            'FIL-USD', 'ICP-USD', 'FLOW-USD', 'APE-USD', 'MANA-USD', 'SAND-USD',
            'CRO-USD', 'HBAR-USD', 'ETC-USD', 'LRC-USD', 'ZEC-USD', 'KCS-USD'
        ]
        
        # DeFi tokens
        self.defi_tokens = [
            'AAVE-USD', 'COMP-USD', 'MKR-USD', 'SNX-USD', 'YFI-USD', 'SUSHI-USD',
            'CRV-USD', 'BAL-USD', 'REN-USD', 'KNC-USD', 'UMA-USD', 'BADGER-USD'
        ]
        
        # Stablecoins
        self.stablecoins = [
            'USDT-USD', 'USDC-USD', 'BUSD-USD', 'DAI-USD', 'TUSD-USD', 'USDP-USD'
        ]
    ...
    async def start(self):
        ...
        try:
            while self.running:
                ...
                await asyncio.sleep(self.config.data_collection.crypto_data_interval or 30)
    ...
    async def _collect_symbol_data(self, symbol: str):
        ...
        try:
            ...
            if data:
                ...
                self.logger.debug(f"Collected crypto data for {symbol}: ${data.get('price', 'N/A')}")
    ...
    async def _fetch_crypto_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        for source_func in sources:
            try:
                ...
                self.logger.debug(f"Source {source_func.__name__} failed for {symbol}: {e}")
                continue
                ...
                if response.status == 200:
                    ...
                    if crypto_id in data:
                        ...
                        return {
                            ...
                            'open': float(crypto_data.get('usd', 0)),  # CoinGecko doesn't provide OHLC in simple API
                            ...
                            'change_24h_percent': float(crypto_data.get('usd_24h_change', 0)),
    ...
    async def get_latest_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data_list = await self.db_manager.get_crypto_data(symbol, start_time, end_time)
            if data_list:
                return data_list[-1]
