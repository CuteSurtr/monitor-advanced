...
class StockDataCollector:
    ...
    def __init__(self, config, db_manager, cache_manager: CacheManager):
        ...
        self.watchlist = [
            ...
            'ADBE', 'CRM', 'NKE', 'KO', 'PEP', 'TMO', 'ABT', 'WMT', 'MRK',
            'PFE', 'TXN', 'AVGO', 'COST', 'ACN', 'DHR', 'LLY', 'VZ', 'CMCSA'
        ]
    ...
    async def _collect_symbol_data(self, symbol: str, market: str):
        ...
        try:
            ...
            if data:
                ...
                self.logger.debug(f"Collected data for {symbol}: {data.get('close', 'N/A')}")
    ...
    async def get_latest_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data_list = await self.db_manager.get_stock_data(symbol, start_time, end_time)
            if data_list:
