...
class CommodityDataCollector:
    ...
    async def start(self):
        ...
        try:
            while self.running:
                ...
                await asyncio.sleep(self.config.data_collection.commodity_data_interval or 300)
    ...
    async def _collect_symbol_data(self, symbol: str):
        ...
        try:
            ...
            if data:
                ...
                self.logger.debug(f"Collected commodity data for {symbol}: ${data.get('price', 'N/A')}")
    ...
    async def get_latest_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data_list = await self.db_manager.get_commodity_data(symbol, start_time, end_time)
            if data_list:
                return data_list[-1]
    ...
    async def get_historical_data(self, symbol: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            return await self.db_manager.get_commodity_data(symbol, start_date, end_date)
        except Exception as e:
