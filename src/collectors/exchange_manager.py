...
class MultiExchangeManager:
    ...
    async def get_trading_hours(self, exchange_code: str) -> TradingHours:
        ...
        try:
            ...
            next_open, next_close = self._calculate_next_session_times(exchange, current_time)
            ...
            for region, exchange_codes in by_region.items():
                ...
                for exchange_code in exchange_codes:
                    ...
                    region_status['exchanges'][exchange_code] = {
                        ...
                        'next_open': trading_hours.next_open.isoformat() if trading_hours.next_open else None,
                        'next_close': trading_hours.next_close.isoformat() if trading_hours.next_close else None,
    ...
    async def _coordinate_data_collection(self):
        ...
        while self.running:
            try:
                ...
                inactive_collectors = set(self.active_collectors.keys()) - set(active_exchanges)
        ...
        while self.running:
            try:
                ...
                for exchange_code in self.exchanges.keys():
                    ...
                    if not is_connected:
                        self.logger.warning(f"Connectivity issue detected for {exchange_code}")
    ...
    def _is_market_open(self, exchange: ExchangeInfo, current_time: datetime) -> bool:
        ...
        if exchange.market_close < exchange.market_open:
            return current_time_only >= exchange.market_open or current_time_only <= exchange.market_close
        else:
            return exchange.market_open <= current_time_only <= exchange.market_close
    
    def _calculate_next_session_times(self, exchange: ExchangeInfo, current_time: datetime) -> tuple:
        ...
        if next_open <= current_time:
            ...
            next_open_dt = datetime.combine(current_date + timedelta(days=1), exchange.market_open)
            next_open = exchange_tz.localize(next_open_dt)
        ...
        if next_close <= current_time:
            ...
            next_close_dt = datetime.combine(current_date + timedelta(days=1), exchange.market_close)
            next_close = exchange_tz.localize(next_close_dt)
    ...
    async def _start_exchange_collector(self, exchange_code: str):
        ...
        try:
            ...
            if exchange.region == ExchangeRegion.US:
                ...
                collector = await self._create_international_collector(exchange_code, symbols)
    ...
    async def _create_international_collector(self, exchange_code: str, symbols: List[str]):
    ...
    async def _get_collection_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            'active_collectors': len(self.active_collectors),
            'collectors': {}
        }
        ...
        for exchange_code, collector_info in self.active_collectors.items():
            stats['collectors'][exchange_code] = {
                ...
                'uptime_seconds': (datetime.now() - collector_info['started_at']).total_seconds(),
                'status': 'active'
            }
