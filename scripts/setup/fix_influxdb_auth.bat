...
docker run -d --name stock_monitor_influxdb --network stock_monitor_default -p 8086:8086 -e DOCKER_INFLUXDB_INIT_MODE=setup -e DOCKER_INFLUXDB_INIT_USERNAME=admin -e DOCKER_INFLUXDB_INIT_PASSWORD=your_admin_password -e DOCKER_INFLUXDB_INIT_ORG=stock_monitor -e DOCKER_INFLUXDB_INIT_BUCKET=market_data -e DOCKER_INFLUXDB_INIT_ADMIN_TOKEN=your_influxdb_token_here -v influxdb_data:/var/lib/influxdb2 influxdb:2.7-alpine
