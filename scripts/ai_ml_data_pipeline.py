...
class AIMLDataPipeline:
    ...
    def stop_pipeline(self):
        ...
        self.logger.info("⏹️ AI/ML Data Pipeline stopped")
    ...
    async def _data_collection_task(self):
        ...
        try:
            self.logger.info("📊 Running data collection task")
            ...
            for symbol in self.symbols:
                try:
                    ...
                    if not latest_data or self._is_data_stale(latest_data):
                        self.logger.warning(f"⚠️ Stale data detected for {symbol}")
    ...
    async def _monitoring_task(self):
        ...
        try:
            self.logger.info("📊 Running monitoring task")
            start_time = datetime.now()
    ...
    async def _validate_models(self, symbol: str, models_performance: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for model_name, perf in models_performance.items():
            ...
            if not is_valid:
                ...
                self.logger.warning(f"⚠️ Model {model_name} failed validation for {symbol}")
            ...
            for symbol, models in self.ai_ml_engine.model_performance.items():
                for model_name, perf in models.items():
                    ...
                    if current_score < threshold:
                        ...
                        self.logger.warning(f"⚠️ Performance degradation detected: {symbol}:{model_name} ({current_score:.3f})")
