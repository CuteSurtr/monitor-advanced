...
def setup_influxdb():
    ...
    try:
        ...
        if response.status_code in [201, 422]:  # 422 means already setup
            print("✓ InfluxDB setup completed or already exists")
...
def get_existing_token():
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            known_token = "xEoh_d1wMmj_8sE2ipM1nht5Iq_iJiziHWVGvt9Haplv8QPDuR6oFTRIcX3rT8rG1M6H21tk0ssTv5ujfrPRDg=="
    ...
    for bucket_data in buckets:
        try:
            ...
            if response.status_code in [201, 422]:  # 422 means already exists
                print(f"✓ Bucket '{bucket_data['name']}' ready")
            else:
    ...
    for bucket in buckets:
        ...
        if bucket_data:
            ...
            try:
                ...
                if response.status_code == 204:
                    print(f"✓ Data written to {bucket}: {len(bucket_data)} lines")
                else:
...
def test_data_query(token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✓ Query test successful: {len(data_lines)} data points found")
            return True
        else:
