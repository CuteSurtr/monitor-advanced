...
def test_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✗ InfluxDB health check failed: {response.text}")
            return False
...
def check_buckets():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✗ Failed to get buckets: {response.status_code} - {response.text}")
            return False
    except Exception as e:
...
def test_simple_query():
    ...
    try:
        ...
        if response.status_code == 200:
            print("\n✓ Basic Flux query works")
            lines = response.text.strip().split('\n')
            data_lines = [line for line in lines if line and not line.startswith('#')]
            print(f"  Found {len(data_lines)} data lines")
            return True
        else:
            print(f"\n✗ Basic query failed: {response.status_code}")
...
def check_bucket_data(bucket_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\n✓ {bucket_name}: {len(data_lines)} data points found")
            if data_lines:
                print(f"  Sample: {data_lines[0][:100]}...")
            return len(data_lines) > 0
        else:
            print(f"\n✗ {bucket_name}: Query failed - {response.status_code}")
...
def check_measurements(bucket_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\n✓ {bucket_name} measurements: {list(set(measurements))}")
            return list(set(measurements))
        else:
            print(f"\n✗ {bucket_name}: Failed to get measurements - {response.status_code}")
            return []
    except Exception as e:
        print(f"\n✗ {bucket_name}: Error getting measurements - {e}")
        return []
