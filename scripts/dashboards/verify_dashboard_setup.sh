...
if [ -f "scripts/dashboards/restart_grafana.sh" ]; then
    echo "✓ Grafana Restart Script (Linux/Mac) - FOUND"
    chmod +x scripts/dashboards/restart_grafana.sh
    echo "  ✓ Made executable"
else
    echo "✗ Grafana Restart Script (Linux/Mac) - MISSING"
fi

if [ -f "scripts/dashboards/restart_grafana.bat" ]; then
...
echo ""
echo "Next Steps:"
echo "1. Run: ./scripts/dashboards/restart_grafana.sh (Linux/Mac)"
echo "2. Or: scripts\\dashboards\\restart_grafana.bat (Windows)"
